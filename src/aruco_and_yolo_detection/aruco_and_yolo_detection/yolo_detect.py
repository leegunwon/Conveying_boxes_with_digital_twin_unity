...
class ImageSubscriber(Node):
    def __init__(self):
        ...
        print(os.getcwd())
        self.model = YOLO('src/aruco_and_yolo_detection/aruco_and_yolo_detection/2nd_B1_best.pt')

        with open("src/aruco_and_yolo_detection/aruco_and_yolo_detection/calibrition.pkl", "rb") as f:
            self.mtx, self.dist = pickle.load(f)

        self.publisher_ = self.create_publisher(CompressedImage, 'yolo/compressed', 1)
        self.info_publisher = self.create_publisher(String, 'yolo/detected_info', 5)  # String 메시지 퍼블리셔
    ...
    def listener_callback_rgb(self, msg):
        ...
        dst = get_calibrated_image(image, self.mtx, self.dist)
        image, result = detect_objects(self.model, dst)
